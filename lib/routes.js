Router.route('/uber', function() {
        var name = this.params.query.user_name;
        var link = fetchUber();
        postMessage(name + ', you have to login: ' + link);
}, {where: 'server'});
...
Router.route('/message', function () {
  ...
  if (text == 'auth') {
    ...
    this.response.end('Not a valid option! Try `auth`, `request startingPoint / endingPoint`, `cancel` or `status`');
  }
}, {where: 'server'});
