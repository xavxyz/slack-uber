Router.route('/uber', {
    name: 'uber',
    path: '/uber',
    action: function() {
        if(this.params.query.code){
            Meteor.call('authUber', this.params.query.code, function(error, result) {
                if (result) {
                    console.log('uber-data', result);
                }
                if (error) {
                    console.log('uber-data-error', error);
                }
            });
        }else{
            Meteor.call('fetchUber', function(error, result) {
                if (result) {
                    console.log('uber-data', result);
                    window.open(result, "_blank", "toolbar=yes, scrollbars=yes, resizable=yes, top=500, left=500, width=500, height=600");
                }
                if (error) {
                    console.log('uber-data-error', error);
                }
            });
        }

    }
});

Router.route('/message', function () {
  var text = this.params.query.text,
      username = this.params.query.user_name;
  if (text == 'auth') {
    postMessage(username + ' auth to Uber');
  } else if (text == 'request') {
    postMessage(username + ' has requested a Uber');
  } else if (text == 'cancel') {
    postMessage(username + ' cancelled his ride!');
  } else if (text == 'status') {
    postMessage(username + " want to know what's up with his Uber");
  } else {
    this.response.end('Not a valid option! Try `auth`, `request`, `cancel` or `status`');
  }
}, {where: 'server'});

